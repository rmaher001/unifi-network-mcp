...
@pytest.mark.asyncio
async def test_stream_network_device_events_filters_by_mac_case_insensitively(tmp_path, monkeypatch) -> None:
    """An uppercase MAC in the path must stream the same events.

    This route fails silently when the filter misses: 200, a valid
    text/event-stream, and then nothing forever, with no signal to the caller
    that the MAC simply did not match.
    """
    monkeypatch.setenv("UNIFI_API_DB_KEY", "k")
    app, key, cid = await _bootstrap(tmp_path, products="network")
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        r = await c.get(
            f"/v1/streams/network/devices/{mac_a.upper()}/events?controller={cid}",
            headers={"Authorization": f"Bearer {key}"},
        )
    assert r.status_code == 200, r.text
    body = r.text
