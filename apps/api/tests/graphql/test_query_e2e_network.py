...
@pytest.mark.asyncio
async def test_client_and_device_lookups_accept_an_uppercase_mac(tmp_path, monkeypatch) -> None:
    """Query.client and Query.device match the controller's lowercase records
    against a MAC given in the form printed on the device label."""
    monkeypatch.setenv("UNIFI_API_DB_KEY", "k")
    app, key, cid = await _bootstrap(tmp_path)

    _stub_managers(
        monkeypatch,
        clients=[{"mac": "aa:bb:cc:dd:ee:01", "hostname": "alpha"}],
        devices=[{"mac": "aa:bb:cc:dd:ee:02", "name": "switch-a"}],
    )
    headers = {"Authorization": f"Bearer {key}"}

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        q_client = f'{{ network {{ client(controller: "{cid}", mac: "AA:BB:CC:DD:EE:01") {{ mac hostname }} }} }}'
        r = await c.post("/v1/graphql", headers=headers, json={"query": q_client})
        body = r.json()
        assert body.get("errors") is None, body
        assert body["data"]["network"]["client"] is not None, "uppercase MAC did not resolve a client"
        assert body["data"]["network"]["client"]["hostname"] == "alpha"

        q_device = f'{{ network {{ device(controller: "{cid}", mac: "AA:BB:CC:DD:EE:02") {{ mac name }} }} }}'
        r = await c.post("/v1/graphql", headers=headers, json={"query": q_device})
        body = r.json()
        assert body.get("errors") is None, body
        assert body["data"]["network"]["device"] is not None, "uppercase MAC did not resolve a device"
        assert body["data"]["network"]["device"]["name"] == "switch-a"


@pytest.mark.asyncio
async def test_client_lookup_still_misses_an_absent_mac(tmp_path, monkeypatch) -> None:
    """Normalizing must not turn the lookup into a match-anything."""
    monkeypatch.setenv("UNIFI_API_DB_KEY", "k")
    app, key, cid = await _bootstrap(tmp_path)
    _stub_managers(monkeypatch, clients=[{"mac": "aa:bb:cc:dd:ee:01", "hostname": "alpha"}])

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        q = f'{{ network {{ client(controller: "{cid}", mac: "11:22:33:44:55:66") {{ mac }} }} }}'
        r = await c.post("/v1/graphql", headers={"Authorization": f"Bearer {key}"}, json={"query": q})
        body = r.json()
        assert body.get("errors") is None, body
        assert body["data"]["network"]["client"] is None
