...
@pytest.mark.asyncio
async def test_edge_device_port_clients_ignores_mac_case(tmp_path, monkeypatch):
    """Device.portClients matches ap_mac to the device MAC; a casing
    difference between the two controller fields must not break the edge."""
    monkeypatch.setenv("UNIFI_API_DB_KEY", "k")
    app, key, cid = await bootstrap(tmp_path, product="network")
    stub_managers(
        monkeypatch,
    ...
    assert body.get("errors") is None, body
    port_macs = {c["mac"] for c in body["data"]["network"]["device"]["portClients"]}
    assert port_macs == {"cc:dd:ee:ff:00:01"}


@pytest.mark.asyncio
async def test_edge_device_port_clients_of_a_mac_less_device_is_empty(tmp_path, monkeypatch):
    """A device record carrying no MAC must own no clients.

    Under a raw `==` this returned EVERY wired client, because a client with
    no ap_mac compares None == None against the device's absent mac.
    """
    monkeypatch.setenv("UNIFI_API_DB_KEY", "k")
    app, key, cid = await bootstrap(tmp_path, product="network")
    stub_managers(
        ...
        {
            ("network", "device_manager", "get_devices"): [
                {"mac": None, "name": "Unadopted", "model": "U6LITE"},
            ],
            ("network", "client_manager", "get_clients"): [
                {"mac": "cc:dd:ee:ff:00:01", "hostname": "wired-a"},
                {"mac": "cc:dd:ee:ff:00:02", "hostname": "wired-b"},
            ],
        },
    )
    body = await graphql_query(
        app,
        key,
        f'''{{ network {{ devices(controller: "{cid}", limit: 5) {{
            items {{ name portClients {{ mac }} }}
        }} }} }}''',
    )
    assert body.get("errors") is None, body
    dev = body["data"]["network"]["devices"]["items"][0]
    assert dev["portClients"] == [], f"a MAC-less device claimed clients: {dev['portClients']}"


@pytest.mark.asyncio
async def test_edge_client_device_ignores_mac_case(tmp_path, monkeypatch):
    """Client.device resolves its AP even when the two records differ in case."""
    monkeypatch.setenv("UNIFI_API_DB_KEY", "k")
    app, key, cid = await bootstrap(tmp_path, product="network")
    stub_managers(
        monkeypatch,
    ...
    body = await graphql_query(
        app,
        key,
        f'''{{ network {{ clients(controller: "{cid}", limit: 5) {{
            items {{ mac device {{ name }} }}
        }} }} }}''',
    )
    assert body.get("errors") is None, body
    item = body["data"]["network"]["clients"]["items"][0]
    assert item["device"] is not None, "uppercase ap_mac did not resolve the AP"
    assert item["device"]["name"] == "Hallway AP"
